...
function concatenateStrings(value1, value2) {
  const concatValue = String(value1).concat(value2);
  return concatValue;
}
...
function getFirstChar(value) {
  const startLetter = String(value.charAt(0));
  return startLetter;
}
...
function startsWith(str, substr) {
  const splitString = str.startsWith(substr);
  return splitString;
}
...
function endsWith(str, substr) {
  const splitString = str.endsWith(substr);
  return splitString;
}
...
function formatTime(minutes, seconds) {
  const formatH = String(minutes).padStart(2, '0');
  const formatM = String(seconds).padStart(2, '0');
...
function containsSubstring(str, substring) {
  const substructCheck = str.includes(substring);
  return substructCheck;
}
...
function countVowels(str) {
  ...
  vowels.forEach((symbol) => {
    const lowerSymbol = symbol.toLowerCase();
    let index = str.indexOf(symbol);
    while (index !== -1 && str[index].toLowerCase() === lowerSymbol) {
      counter += 1;
...
function isPalindrome(str) {
  const punctRemoover = str
    .replace(/[^\w\s]|_/g, '')
    .replace(/\s+/g, '')
    .toLowerCase();
  const reversStr = punctRemoover.split('').reverse().join('');
  const palindromeCheck = punctRemoover === reversStr;
...
function getCardId(value) {
  ...
  const cardIndex = deck.indexOf(value);
